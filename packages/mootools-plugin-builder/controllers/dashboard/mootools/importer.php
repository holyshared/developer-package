...
class JSONResponse {
	...
	public function flush($message = null, $parameters = array(), $status = true) {
...
class DashboardMootoolsImporterController extends Controller {
	...
	public function step5() {
		...
		$files	= $this->_getJSFiles($repos);
		$result = $this->_importJS($file, $files);
	...
	private function _getJSFiles($repos) {
		$u = new User();
		$fs = FileSet::createAndGetSet($repos, 0, $u->getUserID());
	...
	private function _importJS($file, $setFiles) {
		$plugin = new PluginArchive();
		$pluginDir = $plugin->install($file);
		if ($pluginDir) {
			...
			if ($dh = opendir($dir)) {
				...
				while (($file = readdir($dh)) !== false) {
					if (filetype($dir.$file) == 'file') {
						if (array_key_exists($file, $setFiles)) {
	...
	private function _addFile($file, $fr = false) {
		...
		$fv->setAttribute(MOOTOOLS_PLUGIN_LICENSE, $meta["license"]);
		$fv->setAttribute(MOOTOOLS_PLUGIN_AUTHORS, join(",", $meta["authors"]));
